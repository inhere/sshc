...
func PlanCommandProxyRun(host Host, command string, opts RunOptions) (CommandProxyRunPlan, error) {
	...
	if viaName == "" {
		return CommandProxyRunPlan{}, fmt.Errorf("command_proxy host %q requires via", HostLogName(host))
	}
	...
	if !ok {
		return CommandProxyRunPlan{}, fmt.Errorf("command_proxy host %q references missing via host %q", HostLogName(host), viaName)
	}
	via := viaEffective.ToHost()
	if IsCommandProxyHost(via) {
		return CommandProxyRunPlan{}, fmt.Errorf("command_proxy host %q via host %q is also command_proxy", HostLogName(host), HostLogName(via))
	}
	if sameHostIdentity(host, via) || strings.TrimSpace(host.Name) == strings.TrimSpace(via.Name) {
		return CommandProxyRunPlan{}, fmt.Errorf("command_proxy host %q cannot use itself as via", HostLogName(host))
	}
